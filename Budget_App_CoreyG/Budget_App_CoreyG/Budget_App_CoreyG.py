...
class Category:
    ...
    def __str__(self):
        ...
        for info in self.ledger:
            descript = info["description"][:23].ljust(23)
            amount = f"{info['amount']:.2f}"
            amount = amount.rjust(7)
            items += descript + amount + "\n"
...
def create_spend_chart(categories):
    ...
    for category in categories:
        ...
        for info in category.ledger:
            if info['amount'] < 0:
                total += abs(info['amount'])
        funds_spent.append(total)
    total_funds_spent = sum(funds_spent)
    percentages = []
    for amount in funds_spent:
        percent = int((amount / total_funds_spent) * 100)
